...
$originalAssetCode = strtoupper(trim($_POST['original_asset_code'] ?? ''));
...
if ($originalAssetCode === '' || $assetCode === '' || $assetName === '' || $depreciationCode === '') {
...
$result = $service->updateAssetLookup($originalAssetCode, [
...
if ($result['success']) {
    $_SESSION['flash_success'] = "Asset type {$assetCode} updated successfully.";
} else {
    $_SESSION['flash_error'] = $result['error'] ?? 'Failed to update asset type.';
}
