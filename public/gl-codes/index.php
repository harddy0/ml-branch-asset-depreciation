...
<div class="mb-4">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="w-full sm:w-56 -mt-5">
            <label for="filter-account-type" class="text-xs font-mono text-slate-400 block mb-2 sm:mb-0">Filter by Normal Balance</label>
            <select id="filter-account-type" class="w-full px-3 py-1.5 border-2 border-slate-100 focus:border-slate-300 rounded-xl text-sm text-slate-700 bg-white">
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="bg-[#ce2216] border-b border-slate-200">
                    ...
                    <th class="text-center text-sm font-black text-white tracking-widest px-6 py-1">Normal Balance</th>
...
<script>
// Emit a custom event to let existing list loader know to re-run filtering
document.getElementById('filter-account-type').addEventListener('change', function () {
    const ev = new CustomEvent('glCodesFilterChange', { detail: { accountType: this.value } });
